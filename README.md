...
Заголовок — жирная «шапка» тоста, подпись приложения — «Windows PowerShell». Тело —
строки под заголовком (компактно, без разделителя, чтобы уместиться в баннер).
...
Пишет в stderr (`CCNOTIFY_ENGINE=console`), для отладки.
